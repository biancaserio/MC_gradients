...
for i in range(len(yeo7_networks_array_perm)):
    ...
    for n in range(7):
        ...
        # define model
        model = sm.MixedLM(endog = sum_of_squares, exog = pd.DataFrame({'estradiol': hormones_28Me_excl26.z_estradiol, 'progesterone': hormones_28Me_excl26.z_progesterone, 'PSS': behavior_28Me_excl26.z_PSS}), groups = hormones_28Me_excl26.Experiment, exog_re=None, exog_vc=None)
    ...
    for n1 in range(7):
        ...
        for n2 in range(7):
            ...
            if n1!=n2 and [n1+1, n2+1] not in pairwise_comparison_order and list(reversed([n1+1, n2+1])) not in pairwise_comparison_order:
                ...
                # define model
                model = sm.MixedLM(endog = BN_dispersion, exog = pd.DataFrame({'estradiol': hormones_28Me_excl26.z_estradiol, 'progesterone': hormones_28Me_excl26.z_progesterone, 'PSS': behavior_28Me_excl26.z_PSS}), groups = hormones_28Me_excl26.Experiment, exog_re=None, exog_vc=None)
