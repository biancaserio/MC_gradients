...
# Runs spin permutation for within and between network dispersion hormonal contrasts with lmer (28&OC data -> all same as 28&Me analyses but without progesterone)
...
# aligned gradient
array_aligned_fc_OC_G1 = np.genfromtxt(resdir+'array_aligned_fc_OC_G1.csv', delimiter=',')

# hormone data
hormones_28OC = pd.read_csv(resdir+'hormones_28OC.csv', index_col = 0)
...
for i in range(len(yeo7_networks_array_perm)):
    ...
    # gradient values
    g1 = array_aligned_fc_OC_G1.T  # transpose to obtain shape (400 x 29) in order to access/index the relevant network nodes
